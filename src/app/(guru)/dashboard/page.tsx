...
import { getStoredHistory, type LkpdHistoryEntry } from "@/lib/lkpd-history";
...
export default function DashboardGuru() {
  ...
  useEffect(() => { const profile=()=>{const p=getStoredTeacherProfile();setTeacherName(p.nama.split(" ")[0]||"Guru");setSchool(p.sekolah)};const loadHistory=()=>setHistory(getStoredHistory());profile();loadHistory();window.addEventListener("teacher_profile_updated",profile);window.addEventListener("lkpd_history_updated",loadHistory);return()=>{window.removeEventListener("teacher_profile_updated",profile);window.removeEventListener("lkpd_history_updated",loadHistory)}},[]);
  ...
  return <div><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold md:text-3xl">Selamat datang, {teacherName}</h1><p className="mt-2 text-[#414753]">Ringkasan data pembelajaran aktual{school?` di ${school}`:""}.</p>{dataMessage&&<p role="status" className="mt-2 text-sm font-semibold text-green-700">{dataMessage}</p>}</div><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={()=>{if(loadDemoData())setDataMessage("Data simulasi berhasil dimuat!")}}>Muat Data Simulasi</Button><Button variant="ghost" className="text-red-700" onClick={()=>setResetOpen(true)}>Kosongkan Data</Button><Button href="/asesmen/buat"><Plus className="h-4 w-4"/>Buat Asesmen Baru</Button></div></div>
  ...
  <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_340px]"><Card><div className="mb-5 flex justify-between"><h2 className="text-xl font-semibold">Aktivitas Terbaru</h2><a className="text-sm font-semibold text-[#0066cc]" href="/riwayat">Lihat Semua</a></div>{activities.length?<div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th className="pb-3">Judul</th><th>Kelas</th><th>Tanggal</th><th>Status</th></tr></thead><tbody>{activities.map(item=><tr className="border-t" key={item.id}><td className="py-4 font-semibold">{item.title}</td><td>{item.className}</td><td>{new Date(item.date).toLocaleDateString("id-ID")}</td><td><span className="rounded-full bg-blue-50 px-3 py-1 text-xs text-blue-700">{item.status}</span></td></tr>)}</tbody></table></div>:<div className="py-12 text-center text-[#6b7280]">Belum ada aktivitas. Buat kelas dan asesmen baru, atau klik &apos;Muat Data Simulasi&apos; untuk demo instan.</div>}</Card><Card className="flex flex-col items-center"><h2 className="self-start text-xl font-semibold">Kesiapan Mengajar</h2><div className="mt-8"><DonutChart label={history.length?"Siap":"Belum ada"} value={readiness}/></div><p className="mt-6 text-center text-sm text-[#414753]">{history.length?`${validated} dari ${history.length} LKPD telah divalidasi.`:"Generate LKPD untuk mulai mengukur kesiapan."}</p></Card></div><ConfirmModal isOpen={resetOpen} title="Kosongkan seluruh data?" description="Seluruh data kelas, asesmen, hasil, riwayat LKPD, dan sesi siswa akan dihapus. Tindakan ini tidak dapat dibatalkan." confirmText="Kosongkan Data" variant="danger" onConfirm={()=>{if(resetAllData())setDataMessage("Seluruh data berhasil dikosongkan.");setResetOpen(false)}} onCancel={()=>setResetOpen(false)}/></div>;
}
